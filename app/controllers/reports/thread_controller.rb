...
class Reports::ThreadController < ApplicationController
  ...
  def index
    ...
    if params[:show_resolved].present? && params[:show_resolved]
      @reports = ThreadReport.all
    else
      @reports = ThreadReport.where(resolved: false) # Only show reports that are not resolved
    end

    serialized_reports = ThreadReportSerializer.new(@reports, include: [:user]).serializable_hash
  ...
  def create
    ...
    @report = ThreadReport.new(thread: Forums::Thread.find(id), user: current_user, reason: params[:reason])

    if @report.save
      hash = ThreadReportSerializer.new(@report).serializable_hash
  ...
  def destroy
    # mark it as resolved, with an action, and send a notification to the reporting user
    unless current_user
      render json: { error: "Not logged in" }, status: 401
      return
    end
    ...
    @report = ThreadReport.find(params[:id])
